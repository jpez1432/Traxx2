...
void CNode::Update(Game::CTrack &Track)
{
    glDeleteLists(GLTerrain, 1);
    glDeleteLists(GLBoxes, 1);
    glDeleteLists(GLWater, 1);
    ...
    GLWater = glGenLists(1);
    glNewList(GLWater, GL_COMPILE);
        glColor4f(0.0f, 0.25f, 1.0f, 1.0f);
        glBegin(GL_QUADS);
        for (unsigned int x = Center.x - LeafSize / 2; x < Center.x + LeafSize / 2; x++) {
            for (unsigned int z = Center.z - LeafSize / 2; z < Center.z + LeafSize / 2; z++) {
                glVertex3f(x + 1.0f, Track.WaterHeight, z);
                glVertex3f(x + 1.0f, Track.WaterHeight, z + 1.0f);
                glVertex3f(x, Track.WaterHeight, z + 1.0f);
                glVertex3f(x, Track.WaterHeight, z);
            }
        }
        glEnd();
    glEndList();
    ...
    for (unsigned int x = 0; x < LeafSize; x++) {
        for (unsigned int z = 0; z < LeafSize; z++) {
            ...
            OffsetX = (Center.x - LeafSize / 2) + x;
            OffsetZ = (Center.z - LeafSize / 2) + z;
            Duplicates.push_back(Terrain->GetTextureIndex(OffsetX, OffsetZ));

            if (Terrain->GetHeight(OffsetX, OffsetZ).y < Track.GetWaterHeight()){
                Reflects = true;
            }
        }
    }
    ...
    GLTerrain = glGenLists(1);
    glNewList(GLTerrain, GL_COMPILE);
    ...
    GLBoxes = glGenLists(1);
    glNewList(GLBoxes, GL_COMPILE);
...
void CNode::Render(bool Overlay)
{
    if (Overlay && Config->Render.Water) {
        glCallList(GLWater);
    }

    if (Overlay == false || Config->Editor.Mode == 0 ) {
    ...
    if (Overlay && Config->Editor.Mode == 1) {
    ...
    } else if (Overlay && Config->Editor.Mode == 2) {
    ...
    } else if (Overlay && Config->Editor.Mode == 4) {
