...
// Consider using _stricmp/_strnicmp under Windows or strcmpi/strncasecmp. We don't actually use either ImStricmp/ImStrnicmp in the codebase any more.
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
    ...
    {
        int current = g.BeginPopupStack.Size;        // Too few or too many EndMenu()/EndPopup()
        if(write) {
            *p_backup = (short)current;
        } else {
