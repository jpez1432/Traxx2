...
bool CTexture::LoadRaw(CPod &Pod, std::string Filename, unsigned int *DefaultPalette)
{
    ...
    Pod.FindFile(Palette, Size, Offset);
...
bool CTexturePool::Create(CPod &Pod, std::vector<std::string> &Filenames)
{
    this->Pod = &Pod;
    ...
    std::string Palette("\\MetalCr2.act");
//    Pod.FindFile(Palette, Size, Offset);
    ...
    for(unsigned int i = 0; i < Textures.size(); i++) {
        ...
        if(Textures[i].LoadRaw(Pod, Filenames[i], DefaultPalette)) {
...
bool CTexturePool::Save(CPod &Pod, std::string Filename)
{
//    int Size, Offset;
...
//    if(Offset != 0) {
//        StreamOut.seekg(Offset, std::ios::beg);
//    }

    StreamOut << Textures.size() << std::endl;
    for (unsigned int i = 0; i < Textures.size(); i++) {
        StreamOut << GetFilename(i) << std::endl;
    }
